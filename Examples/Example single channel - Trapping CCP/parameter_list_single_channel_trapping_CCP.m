...
parameter.state_vec_fields={'inCCP','trapped','on_membrane'}; % which fields of list_states are used in which order for state number attribution
...
parameter.state{2,2}={[0;0;1],[1;0;1]};
parameter.state{2,3}={[],[]};

% trapped outside CCP
parameter.state{3,1}="Confined";
parameter.state{3,2}={[0;1;1]};
parameter.state{3,3}={[]};

% trapped inside CCP
parameter.state{4,1}="Trapped alone in CCP";
parameter.state{4,2}={[1;1;1]};
parameter.state{4,3}={[]};
